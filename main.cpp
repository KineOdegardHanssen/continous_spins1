...
int main()   // main. Monte Carlo steps here?
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(isotropic)
        {
            ...
            {
                k = mylattice.sites[i].bonds[j].siteindex2; // Hope I can actually get to this value.
                //                                          // I could, alternatively, just store the index
                //                                          // of bonds. But then I need to organize the bonds
                //                                          // and that is such a hassle.
                J = mylattice.sites[i].bonds[j].J;
                sx = mylattice.sites[k].spinx;
                sy = mylattice.sites[k].spiny;
                sz = mylattice.sites[k].spinz;
                partnerspinx += J*sx;
                partnerspiny += J*sy;
                partnerspinz += J*sz;
            }
